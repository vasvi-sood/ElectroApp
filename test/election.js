...
contract("Election", function(accounts) {
  ...
  it("Candidates inforamtion is correct", function(){
  	return Election.deployed().then(function (instance) {
  		electionInstance=instance;
  		return instance.candidates(1);
  	}).then(function(candidate1){
      assert.equal(candidate1[0],1,"correct id");
      assert.equal(candidate1[1],"Candidate 1","correct name");
      assert.equal(candidate1[2],0,"correct vote count");
      //try with instance as well-will not work
       return electionInstance.candidates(2);
         
       }).then(function(candidate2){
          assert.equal(candidate2[0],2,"correct id");
      assert.equal(candidate2[1],"Candidate 2","correct name");
